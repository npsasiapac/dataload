...
AND    t2.column_name like '%LEGACY_REF'
MINUS
...
AND    i1.index_name   like 'LEGACY%'
AND    i1.table_name   like 'DL%'
AND    i1.column_name  like '%DLB_BATCH_ID'
AND    i2.column_name  like '%LEGACY_REF'
...
l_string:=NULL;
l_ind   := SUBSTR('LEGACY'||l_i||'_'||p1.table_name,1,30);
--
l_string := 'CREATE INDEX '||l_ind
               ||' ON '
               ||p1.table_name
               ||'('
               ||p1.seqno_col
